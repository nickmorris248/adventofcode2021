...
def path_finder(file_name):
    ...
    for path in paths_list:
        ...
        for cave in paths[path]:
            ...
            if (cave not in new_path) or ('*' not in new_path) or (cave.isupper()):
                # Add cave to the path
                # Start and end can't be added twice
                if (cave in ('start', 'end')) and cave in new_path:
                    continue
                # If adding a small cave for the second time, add * to mark it
                if (not cave.isupper()) and (cave in new_path):
                    new_path = '*' + new_path + cave
                else:
                    new_path += cave
