...
def snail_maths_solver(file_name):
    ...
    print(f'Homwork with ints: {homework}')

    # Testing
    testing_list = '[[[[[9,8],1],2],3],4]'
    ...
    for idx_line, snail_num in enumerate(homework):
        ...
        # Skip the first line
        if idx_line == 0:
            previous_line = homework[0]
            continue

        added_line = ["["] + previous_line + [","] + snail_num + ["]"]
        print(f'Previous line: {"".join(str(i) for i in previous_line)}\n'
              f'Adding this line: {"".join(str(i) for i in snail_num)}')
